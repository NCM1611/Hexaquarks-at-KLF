{TFile fileOutput1("ds_BW_sigma_pi_branch.root","recreate");

TH1F* ds_proton_pi_Sigma_pi_hist= new TH1F("ds_proton_pi_Sigma_pi","BW of ds",800,2,3.3);
...
std::unique_ptr<TFile> file2( TFile::Open("Sigma_sigma_pi_1D.root") );
if (!file2 || file2->IsZombie()) {
   std::cerr << "Error opening file" << endl;
   exit(-1);
}
std::unique_ptr<TH1> Sigma_dist(file2->Get<TH1>("Sigma_plus_Sigma_pi_1D"));
...
//Defining Particles
TLorentzVector ds;
TLorentzVector *proton,*pi_1,*Sigma,*pi_2;
double_t Masses_1[1] = {ds_mass/*2.474*/};
double_t Masses_2[4] = {0.939,0.139,1.189,0.134};
TLorentzVector V1;
...
//ROOT::Math::Interpolator::Eval is function needed to be used
...
//  cout<<Phasespace_Weight_1<<endl;
  ...
  ds_proton_pi_Sigma_pi_hist->Fill(ds_mass,ds_Weight);

}//End of loop }

//NORMALISING THE WEIGHTING TO CREATE THE WIDTH
  double ds_width = ds_proton_pi_Sigma_pi_hist->Interpolate(2.474);
//  cout<<ds_width<<endl;
  double del_sig_width = ds_proton_pi_Sigma_pi_hist->Interpolate(1.232+1.385);
//  cout<<(0.115+0.036)/del_sig_width<<endl;
  ds_proton_pi_Sigma_pi_hist->Scale((0.115+0.036)/del_sig_width);
...
ds_proton_pi_Sigma_pi_hist->SetTitle("Breit-Wigner distribution of ds (p #pi^{0} #Sigma #pi branch);Masses[GeV/c^{2}];Width");
ds_proton_pi_Sigma_pi_hist->Draw("hist");
