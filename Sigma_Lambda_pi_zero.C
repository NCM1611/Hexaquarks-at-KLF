...
TH2D* Sigma_plus_dist=new TH2D("Sigma_plus_dist","Breit-Wigner Distribution of #Sigma^{*+}",800,1,1.8,800,0,2);
TH1D* Sigma_plus_lambda_dist_1D=new TH1D("Sigma_plus_dist_1D","Breit-Wigner Distribution of #Sigma^{*+}",800,1,1.8);
...
Double_t Masses_1[2] = {1.115,0.134};
  ...
  for (Long64_t j=1;j<801;j++){
    for(Long64_t k=1;k<801;k++){
      if(Sigma_plus_dist->GetBinContent(j,k)>Val){
        ...
        Sigma_plus_lambda_dist_1D->SetBinContent(j,bin_val);
      }
    }
  }
...
Sigma_plus_lambda_dist_1D-> SetTitle("Distribution Sigma Decay (#Lambda #pi branch, 1D); Mass [GeV/c^{2}]; Weight");

Sigma_plus_lambda_dist_1D->Draw("");
