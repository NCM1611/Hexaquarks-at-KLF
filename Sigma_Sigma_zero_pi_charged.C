...
TH1D* Sigma_zero_dist_1D=new TH1D("Sigma_zero_Sigma_pi_1D","Breit-Wigner Distribution of #Sigma^{*0} (#Sigma #pi branch 1D)",800,1,1.8);
TRandom3 rndgen;
...
Double_t Masses_1[2] = {1.192,0.139}; //Sigma masses average to around 1190 MeV
  ...
  for (Long64_t j=1;j<801;j++){
    for(Long64_t k=1;k<801;k++){
      if(Sigma_plus_dist->GetBinContent(j,k)>Val){
        ...
        Sigma_zero_dist_1D->SetBinContent(j,bin_val);
      }
    }
  }
...
Sigma_zero_dist_1D-> SetTitle("Breit-Wigner Distribution of #Sigma^{*} (#Sigma 0 #pi +- decay branch, 1D); Mass [GeV/c^{2}]; Breit-Wigner Value");
//Sigma_plus_dist-> SetTitle("Width Test Plot of #Sigma^{*+} (#Sigma^{+} #pi^{0} branch); Mass [GeV/c^2]; Width [GeV/c^2]");

Sigma_zero_dist_1D->Draw("");
